...
from . import edit_delete, codex, logging
...
autopic_path = os.path.join(os.getcwd(), "Test", "original_pic.png")
digitalpic_path = os.path.join(os.getcwd(), "Test", "digital_pic.png")
autophoto_path = os.path.join(os.getcwd(), "Test", "photo_pfp.png")
    ...
    while DIGITALPICSTART:
        if not os.path.exists(digitalpic_path):
            ...
            while not downloader.isFinished():
                pass
        zedfont = gvarstatus("DEFAULT_PIC") or "Test/helpers/styles/Papernotes.ttf" #Code by T.me/zzzzl1l
        ...
        try:
            ...
            i += 1
            await codex(functions.photos.UploadProfilePhotoRequest(file))
...
async def autoname_loop():
    while AUTONAMESTART := gvarstatus("autoname") == "true":
        ...
        LOGS.info(name)
        try:
            await codex(functions.account.UpdateProfileRequest(first_name=name))
...
async def autobio_loop():
    ...
    while AUTOBIOSTART:
        ...
        LOGS.info(bio)
        try:
            await codex(functions.account.UpdateProfileRequest(about=bio))
        except FloodWaitError as ex:
            LOGS.warning(str(ex))
            await asyncio.sleep(ex.seconds)
        await asyncio.sleep(CHANGE_TIME)
...
codex.loop.create_task(digitalpicloop())
codex.loop.create_task(autoname_loop())
codex.loop.create_task(autobio_loop())
