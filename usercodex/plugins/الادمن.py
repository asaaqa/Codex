...
NO_ADMIN = "❂ : أحتاج الى صلاحيات مشرف لتنفيذ ذالك "
NO_PERM = "❂ : ليس لدي صلاحيات كافية في هذا الجروب "
CHAT_PP_CHANGED = "❂ : تم تغيير صورة الدردشة"
INVALID_MEDIA = "❂ : صيغة غير صحيحه"
...
@codex.cod_cmd(
    ...
    info={
        ...
        "الاوامر": [
            ...
            "{tr}صوره حذف بالرد على صوره بالجروب",
        ],
    },
    groups_only=True,
    require_admin=True,
    ...
    if flag == "-s":
        ...
        if photo:
            try:
                ...
                return await edit_delete(event, f"**خطاء : **`{str(e)}`")
            process = "❂ : تم وضع الصوره بنجاح... "
    else:
        try:
            await event.client(EditPhotoRequest(event.chat_id, InputChatPhotoEmpty()))
        except Exception as e:
            return await edit_delete(event, f"**خطاء : **`{str(e)}`")
        process = "❂ : تم حذف الصوره بنجاح... "
        await edit_delete(event, "\n\n.{process}.❂ :**عزيزي المستخدم **")
    if BOTLOG:
...
@codex.cod_cmd(
    ...
    info={
        ...
        "الوصف": "Provides admin rights to the person in the chat\
...
async def promote(event):
    ...
    if not user:
        return
    codevent = await edit_or_reply(event, "❂ : جاري رفعه مشرف  .... ")
    try:
...
@codex.cod_cmd(
    ...
    info={
        ...
        "وصف": "يزيل جميع الصلاحيات لدى المشرف \
            \nملاحظة : أنت بحاجة إلى ان  تكون لديك كل الصلاحيات  لهذا ويجب أيضًا أن تكون المالك أو المسؤول الذي قام برفع هذا المشرف ",
        "شرح الاوامر": [
            "{tr}تنزيل مشرف  بالرد على المسؤل المردا تنزيله",
            "{tr}تنزيل مشرف + ايدي المسؤل او معرف المسؤل المردا تنزيله",
        ],
    },
    groups_only=True,
    require_admin=True,
...
async def demote(event):
    "لتنزيل مشرف من الجروب"
    user, _ = await get_user_from_event(event)
    if not user:
        return
    codevent = await edit_or_reply(event, "`Demoting...`")
    ...
    try:
        ...
        return await codevent.edit(NO_PERM)
    await codevent.edit("`Demoted Successfully! Betterluck next time`")
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID,
            f"#DEMOTE\
...
async def _ban_person(event):
   ...
   if user.id == 6218149232 or user.id == 3218149232 or user.id == 8218149232:
...
async def nothanos(event):
    ...
    if not user:
        return
    codevent = await edit_or_reply(event, "❂ :**جاري الغاء حظره**   .... ")
    try:
        await event.client(EditBannedRequest(event.chat_id, user.id, UNBAN_RIGHTS))
        await codevent.edit(
            f"{_format.mentionuser(user.first_name ,user.id)} `is تم الغاء حظره . لا تكرر غلطك ولاك .`"
        )
        if BOTLOG:
            await event.client.send_message(
                BOTLOG_CHATID,
        ...
        await codevent.edit("`Uh oh my unban logic broke!`")
    except Exception as e:
        await codevent.edit(f"**Error :**\n`{e}`")
...
async def endmute(event):
    ...
    if not user:
        return
    codevent = await edit_or_reply(event, "`Kicking...`")
    try:
        await event.client.kick_participant(event.chat_id, user.id)
    except Exception as e:
        return await codevent.edit(NO_PERM + f"\n{str(e)}")
    if reason:
        await codevent.edit(
            f"`Kicked` [{user.first_name}](tg://user?id={user.id})`!`\nReason: {reason}"
        )
    else:
        await codevent.edit(f"`Kicked` [{user.first_name}](tg://user?id={user.id})`!`")
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID,
            "#KICK\n"
...
async def _iundlt(event):  # sourcery no-metrics
    ...
    codevent = await edit_or_reply(event, "`Searching recent actions .....`")
    ...
    if not flag:
        ...
        await edit_or_reply(codevent, deleted_msg)
    else:
        main_msg = await edit_or_reply(codevent, deleted_msg)
        for msg in adminlog:
            ruser = (
                await event.client(GetFullUserRequest(msg.old.from_id.user_id))
            ).user
