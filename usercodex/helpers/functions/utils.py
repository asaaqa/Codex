...
async def admin_groups(codex):
    codgroups = []
    async for dialog in codex.iter_dialogs():
...
async def extract_time(cod, time_val):
    if any(time_val.endswith(unit) for unit in ("s", "m", "h", "d", "w")):
        unit = time_val[-1]
        time_num = time_val[:-1]  # type: str
        if not time_num.isdigit():
            await cod.edit("Invalid time amount specified.")
            return None
        if unit == "s":
