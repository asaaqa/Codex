...
class AmbVC:
    ...
    async def join_vc(self, chat, join_as=None):
        if self.CHAT_ID:
            return f"**- المساعد منظم بالمحادثـه الصـوتيـه عـلى** {self.CHAT_NAME}"
        if join_as:
            try:
        ...
        try:
            await self.app.join_group_call(
                chat_id=chat.id,
                stream=AudioPiped("ambratwr/asaaq/Silence01s.mp3"),
        ...
        return f"**- تم انظمام المسـاعد بنجـاح الى المحادثـه الصـوتيـه** **{chat.title}**{join_as_title}"
        ...
        if not self.PLAYLIST:
            if self.PLAYING:
                await self.app.change_stream(
                    self.CHAT_ID,
                    AudioPiped("ambratwr/asaaq/Silence01s.mp3"),
                )
            self.PLAYING = False
