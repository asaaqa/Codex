...
async def tg_dl(event):
    "بالرد على الملف المراد تحميله  "
    mone = await edit_or_reply(event, "**•- جـــآرِ آلتحـمـيل 🔁…. **")
    name = NAME
    path = None
    ...
    if reply:
        ...
        if (
            ...
            await reply.download_media(
                ...
                progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                    progress(d, t, mone, c_time, "**•- جـــآرِ آلتحـمـيل 🔁…. **")
                ),
            )
        ...
        await mone.edit(
            f"** ❆╎تم التحميـل خلال {ms} ثانيـه.**\n** ❆╎مسار التنزيل :- **  `{os.path.relpath(file_name,os.getcwd())}`\n"
        )
